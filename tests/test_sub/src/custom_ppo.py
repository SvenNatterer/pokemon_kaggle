...
class PokemonTCGRecurrentPolicy(RecurrentMultiInputActorCriticPolicy):
    ...
    def _get_action_dist_from_latent_with_mask(self, latent_pi, mask):
        mean_actions = self.action_net(latent_pi)
        huge_negative = torch.tensor(-1e8, device=mean_actions.device)
        bool_mask = mask.bool().to(mean_actions.device)
        masked_logits = torch.where(bool_mask, mean_actions, huge_negative)
    ...
    def forward(self, obs, lstm_states, episode_starts, deterministic=False):
        ...
        distribution = self._get_action_dist_from_latent_with_mask(latent_pi_mlp, mask)
        
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        values = self.value_net(latent_vf_mlp)
        
        # Pack the lstm states back into the same type as the input
        out_lstm_states = type(lstm_states)(lstm_states_pi, lstm_states_vf)
        
        return actions, values, log_prob, out_lstm_states
