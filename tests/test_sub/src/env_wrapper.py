...
class PokemonTCGEnv(gym.Env):
    ...
    def _compute_dense_reward(self, old_obs, new_obs, done):
        ...
        # 4. Deck shrink REWARD (wir wollen, dass das Modell Karten zieht!)
        delta_deck_0 = old_p0.deckCount - new_p0.deckCount
        if delta_deck_0 > 0:
            reward += delta_deck_0 * 0.005  # +0.005 pro gezogener Karte
    ...
    def _get_info(self):
        ...
        try:
            ...
            if obs and obs.current:
                ...
                elif (not p0.active or p0.active[0] is None) and all(b is None for b in p0.bench):
                    info['win_reason'] = 'benchout'
                elif (not p1.active or p1.active[0] is None) and all(b is None for b in p1.bench):
                    info['win_reason'] = 'benchout'
                else:
