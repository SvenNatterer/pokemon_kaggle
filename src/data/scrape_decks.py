...
# Try to import our api
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
try:
    from src.cg.api import all_card_data
except ImportError:
    print("Could not import all_card_data from src.cg.api")
    sys.exit(1)

def scrape_deck(url, deck_id):
    import ssl
    ...
    # Extract <span class="card-count">4</span>
    ...
    print(f"Found {len(matches)} distinct cards. Matching with database...")
    
    cards = all_card_data()
    name_map = {c.name.lower(): c.cardId for c in cards}
    
    # Basic Energy replacements for common ones
    # Limitless uses "Basic {P} Energy" style or just "Psychic Energy"
    ...
    # Handle weird apostrophes
    # Limitless uses ' (straight) or sometimes HTML encoded. 
    # Our DB uses ’ (curly)
    for c in cards:
        if "’" in c.name:
    ...
    with open(f"decks/deck_{deck_id}.csv", "w") as f:
        for cid in deck_list:
            f.write(f"{cid}\n")
            
    print(f"Successfully saved deck_{deck_id}.csv with {len(deck_list)} cards.")
    return True

def main():
    urls = [
        "https://limitlesstcg.com/decks/list/28252", # Crustle
        "https://limitlesstcg.com/decks/list/28253", # Dragapult Blaziken
        "https://limitlesstcg.com/decks/list/28254", # Rocket's Mewtwo
        "https://limitlesstcg.com/decks/list/28255"  # Dragapult
    ]
    
    # We start at deck_id 5 to not overwrite 1-4
