"""Archaludon ex + Cinderace — Compact stable booster
...
try:
    from src.cg.api import all_attack
...
def _update_opp_attack_tracking(obs):
    ...
    for entry in (getattr(obs, "logs", None) or []):
...
def choose_options(obs):
    ...
    max_count = int(getattr(obs.select, "maxCount", min_count or 1) or (min_count or 1))
    max_count = max(min_count, min(max_count, len(scored)))

    selected = []
    for score, i, reason in scored:
        if len(selected) >= max_count:
            break
        if score < 0 and len(selected) >= min_count:
            continue
        selected.append(i)

    if len(selected) < min_count:
        selected = [i for _, i, _ in scored[:min_count]]
