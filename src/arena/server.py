...
from src.arena.arena_core import ArenaStore, EVALUATION_FILE, atomic_write_json, deck_id_for_path, discover_participants, rank_participants, read_json, utc_now
from src.arena.arena_match import load_holdout_results, schedule_replay
from src.league.model_paths import resolve_deck_model_base, resolve_deck_model_path
...
def replay_location(rel_path: str) -> tuple[str, str, str]:
    parts = rel_path.split("/")
    if len(parts) >= 3 and parts[0] == "replays" and parts[1] == "kaggle":
        return ("kaggle", "Kaggle", f"Submission {parts[2]}")
    if len(parts) >= 3 and parts[0] == "replays" and parts[1] == "test":
        return ("test", "Tests", parts[2])
    if len(parts) >= 2 and parts[0] == "replays" and parts[1] == "arena":
        return ("arena", "Arena", "Arena")
    return ("other", "Other", "Replays")


def replay_details(data: dict) -> tuple[dict, int, str, str]:
    info = data.get("info") or {}
    team_names = info.get("TeamNames") or ["P0", "P1"]
    metadata = {
        "p0_name": team_names[0] if len(team_names) > 0 else "P0",
        "p1_name": team_names[1] if len(team_names) > 1 else "P1",
    }
    if "EpisodeId" in info:
        metadata["episode_id"] = info["EpisodeId"]
    ...
    if len(rewards) >= 2:
        ...
        result = "Unknown"
    statuses = data.get("statuses") or ["UNKNOWN", "UNKNOWN"]
    status = " / ".join(str(s) for s in statuses)
    return metadata, snapshots, result, status


def kaggle_submission_descriptions() -> dict[str, str]:
    meta_path = os.path.join(BASE_DIR, "replays", "kaggle", "metadata.json")
    try:
        with open(meta_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        subs = data.get("submissions") or {}
        return {str(k): str(v.get("description", "")) for k, v in subs.items() if isinstance(v, dict)}
    except Exception:
        return {}
...
def start_evaluation():
    ...
    if not (os.environ.get("PYTEST_CURRENT_TEST") or app.testing or app.config.get("TESTING")) and not os.path.exists(os.path.join(BASE_DIR, holdout_file)):
    ...
    if not selected or any(
        ...
        or item.bot_type not in {"ppo", "rule_based"} or not item.model_path
    ...
    cmd = [sys.executable, "-m", "src.arena.evaluation_worker", "--bot-id", bot_id, "--games", str(games), "--holdout-file", holdout_file, "--workers", str(os.cpu_count() or 4)]
    for item in selected:
        if item.bot_type == "rule_based":
            spec = json.dumps({"bot_type": "rule_based", "model_path": item.model_path, "deck_path": item.deck_path})
            cmd.extend(["--candidate-spec", spec])
        else:
            cmd.extend(["--model", item.model_path])
    try:
...
def promote_champion():
    ...
    selection = read_json(os.path.join(BASE_DIR, selection_file), {}) if os.path.exists(os.path.join(BASE_DIR, selection_file)) else read_json(selection_file, {})
    if selection.get("candidate_spec", {}).get("bot_type") == "rule_based":
        return jsonify({"success": False, "message": "Rule-based candidates cannot be promoted as champion model archives."}), 400
    ...
    if result.returncode:
        return jsonify({"success": False, "message": str(result.stderr or result.stdout).strip()}), 400
    return jsonify({"success": True, "message": str(result.stdout or "Champion promoted.").strip(), "champion": read_json(CHAMPION_FILE, {})})
