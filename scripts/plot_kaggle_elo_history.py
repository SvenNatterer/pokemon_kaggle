#!/usr/bin/env python3
"""
Fetch match-by-match Elo histories for the last 5 completed Kaggle submissions,
generate progress plots, and write a markdown report in the reports directory.
"""
...
for sub_id, info in all_data.items():
    ...
    plt.tight_layout()
    img_path = os.path.join(reports_dir, f"kaggle_elo_{sub_id}.png")
...
report_path = os.path.join(reports_dir, "kaggle_elo_report.md")
with open(report_path, "w", encoding="utf-8") as f:
    f.write("# Kaggle Elo History Report\n\n")
    f.write(f"Generated at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    f.write("Detaillierter Verlauf der letzten 5 completed Submissions auf Kaggle.\n\n")
    
    f.write("## 1. Gesamtvergleich\n\n")
    f.write("![Combined Elo Comparison](kaggle_elos_combined.png)\n\n")
    
    f.write("## 2. Einzelstatistiken & Details\n\n")
    ...
    for sub_id, info in all_data.items():
        ...
        f.write(f"### Submission {sub_id} - {desc}\n\n")
        f.write(f"- **Final Elo**: {scores[-1]:.1f}\n")
        f.write(f"- **Peak Elo**: {max(scores):.1f}\n")
        f.write(f"- **Spiele gesamt**: {stats['total']} (Siege: {stats['wins']} | Niederlagen: {stats['losses']})\n")
        f.write(f"- **Winrate**: {wr:.1f}%\n\n")
        f.write(f"![Elo Verlauf {sub_id}](kaggle_elo_{sub_id}.png)\n\n")
        f.write("---\n\n")
