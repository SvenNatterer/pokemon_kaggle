...
MODEL_NAME="${MODEL_NAME:-models/ppo_v4_deck_bank_18_robust.zip}"
...
STAGE3_STEPS="${STAGE3_STEPS:-300000}"
STAGE4_STEPS="${STAGE4_STEPS:-150000}"

# Default opponents (user can override via environment variables)
BEST_OPP_MODEL="${BEST_OPP_MODEL:-models/backup/ppo_v4_deck_bank_18.zip}"
WEAK_OPP_MODEL="${WEAK_OPP_MODEL:-models/backup/ppo_v4_deck_bank_10_opp_stage1.zip}"
...
usage() {
  cat <<'EOF'
Usage: scripts/train_robust_curriculum.sh [options]

Curriculum for robust models, training against Heuristics, Self-Play, the Best Model, and Weak Models.

Options:
  --deck PATH              Target deck csv.
  --model-name PATH        Target model zip/base path.
  --base-model PATH        Start training from this existing model (copies it to model-name).
...
Useful environment overrides:
  STAGE1_STEPS=250000 STAGE2_STEPS=400000 STAGE3_STEPS=300000 STAGE4_STEPS=150000
  NUM_ENVS=8 BATCH_SIZE=1024 N_EPOCHS=4 BELIEF_DIM=64 AUX_COEF=0.10
  BEST_OPP_MODEL=models/backup/ppo_v4_deck_bank_18.zip
  WEAK_OPP_MODEL=models/backup/ppo_v4_deck_bank_10_opp_stage1.zip
EOF
}
...
run_train_stage() {
  ...
  echo "opponent_model=${opp_model:-environment fallback (heuristic/random)}"
  echo "================================================="
  printf ' %q' "${cmd[@]}"
  echo
  ...
  if [ "$DRY_RUN" = "0" ]; then
    ...
    if [ "$RUN_EVAL" = "1" ]; then
      ...
      local eval_log="logs/eval_${MODEL_STEM}_stage${stage}_${ts}.json"
      mkdir -p logs
      echo ""
      echo "Running intermediate reference evaluation for stage ${stage}..."
      echo "  Results will be saved to: ${eval_log}"
      python3 scripts/evaluate_overnight_references.py \
        --candidate "$MODEL_ZIP" \
        --games "$EVAL_GAMES" \
        --results-file "$eval_log"
    fi
  fi
}
...
echo "================================================="
echo "Robust Curriculum (Heuristic -> Self -> Best -> Weak)"
echo "Target deck:  ${DECK}"
echo "Target model: ${MODEL_NAME}"
echo "Base model:   ${BASE_MODEL:-None}"
echo "Stages:       ${START_STAGE}-${END_STAGE}"
echo "================================================="

if [ -n "$BASE_MODEL" ] && [ ! -f "$MODEL_ZIP" ]; then
  require_file "$BASE_MODEL" "base model"
  echo "Copying base model ${BASE_MODEL} to ${MODEL_ZIP} to continue training..."
  cp "$BASE_MODEL" "$MODEL_ZIP"
fi

# Stage 1: Train against a genuinely stochastic fallback opponent.
run_train_stage 1 "vs_random" "$STAGE1_STEPS" "$DECK" "" \
  "8e-5" "0.010" "0.18" "0.040" "0"

# Stage 2: Train against own old checkpoint (Self-Play).
# We take a snapshot of the model after Stage 1 and train against it.
if should_run_stage 2; then
  SELF_STAGE2_MODEL="$(snapshot_current_model 2 "self_stage2")"
else
  SELF_STAGE2_MODEL=""
fi
run_train_stage 2 "vs_self_play" "$STAGE2_STEPS" "$DECK" "$SELF_STAGE2_MODEL" \
  "5e-5" "0.006" "0.15" "0.035" "0"

# Stage 3: Train against the best model.
# This prevents the model from forgetting how to beat strong ladder opponents.
run_train_stage 3 "vs_best_model" "$STAGE3_STEPS" "$DECK" "$BEST_OPP_MODEL" \
  "3e-5" "0.004" "0.12" "0.030" "1"

# Stage 4: Train against weaker or randomized opponents.
# This increases robustness against non-standard/unpredictable plays.
run_train_stage 4 "vs_weak_model" "$STAGE4_STEPS" "$DECK" "$WEAK_OPP_MODEL" \
